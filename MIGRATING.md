...
`MPListenerProtocol` and `MPListenerController` have been removed from the SDK's public API. Media events are now delivered via a closure property on `MPMediaSession`.

#### What Has Changed

- `MPListenerProtocol` has been removed
- `MPListenerController` has been removed; `addSdkListener` and `removeSdkListener` are no longer available
- Media events are now surfaced via the `mediaEventListener` closure property on `MPMediaSession`
...
```swift
class MyClass: MPListenerProtocol {
    ...
    // MPListenerProtocol implementation
    func onMParticleEvent(_ mpEvent: MPEvent) {
        // handle event
    }
}
```
...
```swift
mediaSession.mediaEventListener = { (event: MPMediaEvent) in
    // handle event
}
```
...
```objective-c
@interface MyClass () <MPListenerProtocol>
@end
...
```objective-c
self.mediaSession.mediaEventListener = ^(MPMediaEvent * _Nonnull event) {
    // handle event
};
```
...
- The `mediaEventListener` closure is set per session instance and does not require any registration or deregistration
- Remove all references to `MPListenerProtocol`, `MPListenerController`, `addSdkListener:`, and `removeSdkListener:` from your codebase
